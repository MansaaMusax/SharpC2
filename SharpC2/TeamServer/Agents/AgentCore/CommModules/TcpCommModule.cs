...
class TcpCommModule : ICommModule
{
    ...
    private void ServerReadCallback(IAsyncResult ar)
    {
        ...
        if (bytesRead > 0)
        {
            ...
            stream.BeginWrite(dataToSend, 0, dataToSend.Length, new AsyncCallback(ServerWriteCallback), state);
        }
    }
    ...
    private void ClientConnectCallback(IAsyncResult ar)
    {
        ...
        var dataToSend = Crypto.Encrypt(outbound);
        stream.BeginWrite(dataToSend, 0, dataToSend.Length, new AsyncCallback(ClientWriteCallback), state);
    }
